...
impl PlatformProfile {
  ...
  pub fn from_string(s: &str) -> Self {
    match s {
      PlatformProfile::LOW_POWER => Self::LowPower,
...
impl fmt::Display for PlatformProfile {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      Self::LowPower => write!(f, "{}", PlatformProfile::LOW_POWER),
      Self::Balanced => write!(f, "{}", PlatformProfile::BALANCED),
      Self::Performance => write!(f, "{}", PlatformProfile::PERFORMANCE),
      Self::Unknown => write!(f, "{}", PlatformProfile::BALANCED),
    }
  }
}
...
impl BatteryStates {
  ...
  pub fn init() -> Result<Self, BatteryStatesError> {
    Ok(Self {
      ...
      battery_capacity: RefCell::new(PersFd::new("/sys/class/power_supply/BAT0/capacity", false)?),
